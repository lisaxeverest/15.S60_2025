...
#Observe that this function automatically works on both a single price, or a vector of prices:
clean_price("$100")
clean_price(raw_listings$price[1:5])

#Vectorized functions are very powerful for manipulating columns of a data frame
#But vectorization is not always automatic
#A common example is when the function involves Boolean conditions:
clean_price2 <- function(price) {
  if(price !="$2000.00"){
    return(as.numeric(gsub('\\$|,', '', price)))}
...
clean_price2("$100")
clean_price2(raw_listings$price[1:5])

#A quick fix is to enforce vectorization by "Vectorize()"
clean_price2<- Vectorize(clean_price2)
as.vector(clean_price2(raw_listings$price[1:5]))
...
########## ----------------------------------------------------------------
# (entering at Hands-on: Verbs and Aggregation)
########## ----------------------------------------------------------------
...
# and our desired Geometry (`geom_point`).  
# We are gluing each new element together with `+` signs.   

# Clean, intuitive, and on the way to being pretty. 
...
# That `factor` essentially tells ggplot to treat `bedrooms` as discrete/categorical
# rather than continuous

# The plot looks like this without factor():
by.rating.bedroom %>%
  ggplot(aes(x=review_scores_rating, y=med.price, color=bedrooms)) +
  geom_point()


# To better understand how aes works, let's try a few other things:

#1. aes can also be specified at geom_point()
by.rating.bedroom %>%
  ggplot(aes(x=review_scores_rating, y=med.price)) +
  geom_point(aes(color=factor(bedrooms)))

#2. aes() is required to match aspects of the graph to **dimensions of data**!
## This won't work -- 
by.rating.bedroom %>%
  ggplot(aes(x=review_scores_rating, y=med.price)) +
  geom_point(color=factor(bedrooms))

#3. But this works -- some aspects of the graph can be specified independent of dimensions of data
by.rating.bedroom %>%
  ggplot(aes(x=review_scores_rating, y=med.price),) +
